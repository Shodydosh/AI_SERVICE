"""Ước tính thời gian cho 1K dataset."""
import sys
...
try:
    ...
    print(f"\n{'='*80}")
    print(f"⏱️  ƯỚC TÍNH THỜI GIAN - 1K DATASET")
    print(f"{'='*80}\n")
    ...
    # Thời gian testing (0.5s per candidate với 1K jobs)
    test_time = TARGET_TESTS * 0.3 if (num_candidates >= TARGET_CANDIDATES and num_jobs >= TARGET_JOBS) else 0
    
    total_time = candidate_time + job_time + test_time
    
    print(f"\n⏳ THỜI GIAN CÒN LẠI:")
    if remaining_candidates > 0:
        print(f"   Indexing Candidates: ~{candidate_time/60:.1f} phút ({candidate_time:.0f}s)")
    if remaining_jobs > 0:
        print(f"   Indexing Jobs:       ~{job_time/60:.1f} phút ({job_time:.0f}s)")
    if test_time > 0:
        print(f"   Testing (200):       ~{test_time/60:.1f} phút ({test_time:.0f}s)")
